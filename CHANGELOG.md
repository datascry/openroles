...
- Bundle-size budgets (`site/.size-limit.json`) defended in CI: FilterTable island ≤ 25 KB gzip, Svelte runtime ≤ 20 KB gzip, all client JS ≤ 50 KB gzip, global CSS ≤ 15 KB gzip. The `size-limit` PR job is now enabled.
- Playwright e2e + a11y harness (`site/playwright.config.ts`, `site/tests/e2e/smoke.spec.ts`): page-chrome smoke, axe critical/serious assertions, FilterTable hydration check, RSS feed content-type check. Runs against `bun --bun astro preview` on chromium + Pixel 7 viewport. The `e2e` PR job is now enabled.
- `parseNumPages` test coverage (CDX `showNumPages` JSON-form fallback) brought above the 95 % per-file threshold; previously uncovered lines 82–88 of `scraper/src/harvest/cdx.ts`.
