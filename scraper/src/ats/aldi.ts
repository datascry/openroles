...
// ALDI public job-search endpoint.
//
// Endpoint: https://careers.aldi.us/api/jobs
// Method: GET
// Query: page, size
//
...
//
// Single-tenant; slug = `aldi`. Small board (typically 50-200
...
const AldiResponse = z
  .object({
    total: z.number().optional(),
    jobs: z.array(AldiJob).optional(),
    results: z.array(AldiJob).optional(),
  })
  .passthrough();

const PAGE_SIZE = 100;
const DEFAULT_MAX_PAGES = 10;
const TENANT_SLUG = "aldi";
const HOST = "careers.aldi.us";
...
export interface ParseAldiJobsInput {
...
function buildCandidate(
  raw: z.infer<typeof AldiJob>,
  input: ParseAldiJobsInput,
  ...
  const jid = jobId({ ats: "aldi", tenant_slug: input.tenant.slug, source_id: sourceId, url });
...
export function parseAldiJobs(input: ParseAldiJobsInput): Job[] {
  const parsed = AldiResponse.parse(input.response);
...
export interface ScrapeAldiOptions {
  readonly tenant: TenantInput;
...
export interface ScrapeAldiOutcome {
  readonly jobs: ReadonlyArray<Job>;
  readonly result: TenantResult;
}

export async function scrapeAldiTenant(opts: ScrapeAldiOptions): Promise<ScrapeAldiOutcome> {
  if (opts.tenant.slug !== TENANT_SLUG) {
    return {
      ...
      result: {
        ...
        status: "dead",
        error: `aldi is a single-tenant ATS, slug must be "${TENANT_SLUG}"`,
        jobs_count: 0,
      },
    };
  }
  ...
  let lastStatus = 0;
  const company = opts.tenant.display_name ?? "ALDI";
  try {
    for (let page = 1; page <= maxPages; page++) {
      ...
      const parsed = AldiResponse.parse(body);
      if (typeof parsed.total === "number") total = parsed.total;
      const pageJobs = parseAldiJobs({
        tenant: opts.tenant,
        company,
        response: body,
