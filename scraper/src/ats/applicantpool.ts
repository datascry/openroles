...
// ApplicantPool hosted boards at `{slug}.applicantpool.com`. The public board
...
// strings, and a canonical `jobUrl` (`https://{slug}.applicantpool.com/jobs/{id}`)
...
// crawl-permitted. The platform publishes a live-tenant sitemap at
// feeds.applicantpool.com/site_map_index.xml, a useful discovery surface.
// Tenant identity = slug (subdomain); no metadata needed.
 ...
 */
export function extractApplicantpoolDomainId(html: string): string | null {
...
export function normalizeApplicantpoolDate(value: string): string | undefined {
...
interface ApplicantpoolJobRecord {
  id?: number | string;
  title?: string | null;
  city?: string | null;
  iso3?: string | null;
...
interface ApplicantpoolResponse {
  success?: boolean;
  data?: {
    jobs?: ReadonlyArray<ApplicantpoolJobRecord>;
  } | null;
}
...
function postedAtFrom(raw: string | null | undefined, observedAt: string): string | undefined {
  ...
  const posted = normalizeApplicantpoolDate(raw);
...
function jobUrlFor(slug: string, sourceId: string, rawUrl: string | undefined): string {
  if (typeof rawUrl === "string" && rawUrl.startsWith("https://")) {
    try {
      ...
      if (
        parsed.username === "" &&
        parsed.password === "" &&
        parsed.hostname.endsWith(".applicantpool.com")
      )
        return rawUrl;
    } catch {
  ...
  return `https://${slug}.applicantpool.com/jobs/${encodeURIComponent(sourceId)}`;
}

export interface ParseApplicantpoolJobsInput {
 ...
 */
export function parseApplicantpoolJobs(input: ParseApplicantpoolJobsInput): Job[] {
  const body = input.response as ApplicantpoolResponse;
  if (body.success !== true || !Array.isArray(body.data?.jobs)) {
    throw new HttpError(
      "transient",
      "applicantpool job list unavailable (success=false or no data.jobs)",
    );
  }
  const out: Job[] = [];
...
export interface ScrapeApplicantpoolOptions {
  readonly tenant: TenantInput;
  readonly client: HttpClient;
  readonly observedAt: string;
}

export interface ScrapeApplicantpoolOutcome {
  readonly jobs: ReadonlyArray<Job>;
  readonly result: TenantResult;
}

export async function scrapeApplicantpoolTenant(
  opts: ScrapeApplicantpoolOptions,
): Promise<ScrapeApplicantpoolOutcome> {
  try {
    assertSafeSlug(opts.tenant.slug);
    // Step 1: GET the board page to discover the per-tenant domain_id.
    const boardUrl = `https://${opts.tenant.slug}.applicantpool.com/jobs/`;
    const boardRes = await opts.client.request(boardUrl);
    const boardHtml = await boardRes.text();
    const domainId = extractApplicantpoolDomainId(boardHtml);
    if (domainId === null) {
    ...
    const apiUrl = `https://${opts.tenant.slug}.applicantpool.com/core/jobs/${domainId}?getParams=%7B%22isInternal%22%3A0%7D`;
    ...
    const jobs = parseApplicantpoolJobs({
      tenant: opts.tenant,
      company,
      response: body,
