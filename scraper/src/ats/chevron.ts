...
// Chevron public job-search endpoint.
//
// Endpoint: https://careers.chevron.com/api/jobs
// Method: GET
// Query: page, size
//
...
//
// Single-tenant; slug = `chevron`. Small board (typically 50-200
...
const ChevronResponse = z
  .object({
    total: z.number().optional(),
    jobs: z.array(ChevronJob).optional(),
    results: z.array(ChevronJob).optional(),
  })
  .passthrough();

const PAGE_SIZE = 100;
const DEFAULT_MAX_PAGES = 10;
const TENANT_SLUG = "chevron";
const HOST = "careers.chevron.com";
...
export interface ParseChevronJobsInput {
...
function buildCandidate(
  raw: z.infer<typeof ChevronJob>,
  input: ParseChevronJobsInput,
...
export function parseChevronJobs(input: ParseChevronJobsInput): Job[] {
  const parsed = ChevronResponse.parse(input.response);
...
export interface ScrapeChevronOptions {
  readonly tenant: TenantInput;
...
export interface ScrapeChevronOutcome {
  readonly jobs: ReadonlyArray<Job>;
  readonly result: TenantResult;
}

export async function scrapeChevronTenant(
  opts: ScrapeChevronOptions,
): Promise<ScrapeChevronOutcome> {
  if (opts.tenant.slug !== TENANT_SLUG) {
    return {
      ...
      result: {
        ...
        status: "dead",
        error: `chevron is a single-tenant ATS, slug must be "${TENANT_SLUG}"`,
        jobs_count: 0,
      },
    };
  }
  ...
  let lastStatus = 0;
  const company = opts.tenant.display_name ?? "Chevron";
  try {
    for (let page = 1; page <= maxPages; page++) {
      ...
      const parsed = ChevronResponse.parse(body);
      if (typeof parsed.total === "number") total = parsed.total;
      const pageJobs = parseChevronJobs({
        tenant: opts.tenant,
        company,
        response: body,
