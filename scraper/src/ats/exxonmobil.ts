...
// ExxonMobil public job-search endpoint.
//
// Endpoint: https://jobs.exxonmobil.com/api/jobs
// Method: GET
// Query: page, size
//
...
const ExxonMobilResponse = z
  .object({
    total: z.number().optional(),
    jobs: z.array(ExxonMobilJob).optional(),
    results: z.array(ExxonMobilJob).optional(),
  })
  .passthrough();

const PAGE_SIZE = 100;
const DEFAULT_MAX_PAGES = 10;
const TENANT_SLUG = "exxonmobil";
const HOST = "jobs.exxonmobil.com";
...
export interface ParseExxonMobilJobsInput {
...
function buildCandidate(
  raw: z.infer<typeof ExxonMobilJob>,
  input: ParseExxonMobilJobsInput,
...
export function parseExxonMobilJobs(input: ParseExxonMobilJobsInput): Job[] {
  const parsed = ExxonMobilResponse.parse(input.response);
...
export interface ScrapeExxonMobilOptions {
  readonly tenant: TenantInput;
...
export interface ScrapeExxonMobilOutcome {
  readonly jobs: ReadonlyArray<Job>;
  readonly result: TenantResult;
}

export async function scrapeExxonMobilTenant(
  opts: ScrapeExxonMobilOptions,
): Promise<ScrapeExxonMobilOutcome> {
  if (opts.tenant.slug !== TENANT_SLUG) {
    return {
      ...
      result: {
        ...
        status: "dead",
        error: `exxonmobil is a single-tenant ATS, slug must be "${TENANT_SLUG}"`,
        jobs_count: 0,
      },
    };
  }
  ...
  let lastStatus = 0;
  const company = opts.tenant.display_name ?? "ExxonMobil";
  try {
    for (let page = 1; page <= maxPages; page++) {
      ...
      const parsed = ExxonMobilResponse.parse(body);
      if (typeof parsed.total === "number") total = parsed.total;
      const pageJobs = parseExxonMobilJobs({
        tenant: opts.tenant,
        company,
        response: body,
