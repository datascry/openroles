...
// Inditex public job-search endpoint.
//
// Endpoint: https://www.inditexcareers.com/api/jobs
// Method: GET
// Query: page, size
//
...
//
// Single-tenant; slug = `inditex`. Small board (typically 50-200
...
const InditexResponse = z
  .object({
    total: z.number().optional(),
    jobs: z.array(InditexJob).optional(),
    results: z.array(InditexJob).optional(),
  })
  .passthrough();

const PAGE_SIZE = 100;
const DEFAULT_MAX_PAGES = 10;
const TENANT_SLUG = "inditex";
const HOST = "www.inditexcareers.com";
...
export interface ParseInditexJobsInput {
...
function buildCandidate(
  raw: z.infer<typeof InditexJob>,
  input: ParseInditexJobsInput,
...
export function parseInditexJobs(input: ParseInditexJobsInput): Job[] {
  const parsed = InditexResponse.parse(input.response);
...
export interface ScrapeInditexOptions {
  readonly tenant: TenantInput;
...
export interface ScrapeInditexOutcome {
  readonly jobs: ReadonlyArray<Job>;
  readonly result: TenantResult;
}

export async function scrapeInditexTenant(
  opts: ScrapeInditexOptions,
): Promise<ScrapeInditexOutcome> {
  if (opts.tenant.slug !== TENANT_SLUG) {
    return {
      ...
      result: {
        ...
        status: "dead",
        error: `inditex is a single-tenant ATS, slug must be "${TENANT_SLUG}"`,
        jobs_count: 0,
      },
    };
  }
  ...
  let lastStatus = 0;
  const company = opts.tenant.display_name ?? "Inditex";
  try {
    for (let page = 1; page <= maxPages; page++) {
      ...
      const parsed = InditexResponse.parse(body);
      if (typeof parsed.total === "number") total = parsed.total;
      const pageJobs = parseInditexJobs({
        tenant: opts.tenant,
        company,
        response: body,
