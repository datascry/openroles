...
// Saudi Aramco public job-search endpoint.
//
// Endpoint: https://careers.aramco.com/api/jobs
// Method: GET
// Query: page, size
//
...
const SaudiAramcoResponse = z
  .object({
    total: z.number().optional(),
    jobs: z.array(SaudiAramcoJob).optional(),
    results: z.array(SaudiAramcoJob).optional(),
  })
  .passthrough();

const PAGE_SIZE = 100;
const DEFAULT_MAX_PAGES = 10;
const TENANT_SLUG = "saudiaramco";
const HOST = "careers.aramco.com";
...
export interface ParseSaudiAramcoJobsInput {
...
function buildCandidate(
  raw: z.infer<typeof SaudiAramcoJob>,
  input: ParseSaudiAramcoJobsInput,
...
export function parseSaudiAramcoJobs(input: ParseSaudiAramcoJobsInput): Job[] {
  const parsed = SaudiAramcoResponse.parse(input.response);
...
export interface ScrapeSaudiAramcoOptions {
  readonly tenant: TenantInput;
...
export interface ScrapeSaudiAramcoOutcome {
  readonly jobs: ReadonlyArray<Job>;
  readonly result: TenantResult;
}

export async function scrapeSaudiAramcoTenant(
  opts: ScrapeSaudiAramcoOptions,
): Promise<ScrapeSaudiAramcoOutcome> {
  if (opts.tenant.slug !== TENANT_SLUG) {
    return {
      ...
      result: {
        ...
        status: "dead",
        error: `saudiaramco is a single-tenant ATS, slug must be "${TENANT_SLUG}"`,
        jobs_count: 0,
      },
    };
  }
  ...
  let lastStatus = 0;
  const company = opts.tenant.display_name ?? "Saudi Aramco";
  try {
    for (let page = 1; page <= maxPages; page++) {
      ...
      const parsed = SaudiAramcoResponse.parse(body);
      if (typeof parsed.total === "number") total = parsed.total;
      const pageJobs = parseSaudiAramcoJobs({
        tenant: opts.tenant,
        company,
        response: body,
