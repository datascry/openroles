...
// 7-Eleven public job-search endpoint.
//
// Endpoint: https://careers.7-eleven.com/api/jobs
// Method: GET
// Query: page, size
//
...
const SevenElevenResponse = z
  .object({
    total: z.number().optional(),
    jobs: z.array(SevenElevenJob).optional(),
    results: z.array(SevenElevenJob).optional(),
  })
  .passthrough();

const PAGE_SIZE = 100;
const DEFAULT_MAX_PAGES = 10;
const TENANT_SLUG = "seveneleven";
const HOST = "careers.7-eleven.com";
...
export interface Parse7eJobsInput {
...
function buildCandidate(
  raw: z.infer<typeof SevenElevenJob>,
  input: Parse7eJobsInput,
...
export function parse7eJobs(input: Parse7eJobsInput): Job[] {
  const parsed = SevenElevenResponse.parse(input.response);
...
export interface Scrape7eOptions {
  readonly tenant: TenantInput;
...
export interface Scrape7eOutcome {
  readonly jobs: ReadonlyArray<Job>;
  readonly result: TenantResult;
}

export async function scrapeSevenElevenTenant(opts: Scrape7eOptions): Promise<Scrape7eOutcome> {
  if (opts.tenant.slug !== TENANT_SLUG) {
    return {
      ...
      result: {
        ...
        status: "dead",
        error: `seveneleven is a single-tenant ATS, slug must be "${TENANT_SLUG}"`,
        jobs_count: 0,
      },
    };
  }
  ...
  let lastStatus = 0;
  const company = opts.tenant.display_name ?? "7-Eleven";
  try {
    for (let page = 1; page <= maxPages; page++) {
      ...
      const parsed = SevenElevenResponse.parse(body);
      if (typeof parsed.total === "number") total = parsed.total;
      const pageJobs = parse7eJobs({
        tenant: opts.tenant,
        company,
        response: body,
