...
// Tata Consultancy Services public job-search API.
//
// Endpoint: https://www.tcs.com/careers/api/jobs
// Method: GET
// Query: page, size
//
// Response shape:
//   { total: <int>, jobs: [{ jobId, title, description, location, country_code, datePosted, url }] }
//
// Single-tenant; slug = `tcs`.
...
const TcsResponse = z
  .object({
    ...
    jobs: z.array(TcsJob).optional(),
    results: z.array(TcsJob).optional(),
  })
  .passthrough();

const PAGE_SIZE = 50;
const DEFAULT_MAX_PAGES = 100;
const TENANT_SLUG = "tcs";
const HOST = "www.tcs.com";

function sourceIdOf(j: z.infer<typeof TcsJob>): string | undefined {
...
export interface ParseTcsJobsInput {
...
function buildCandidate(
  raw: z.infer<typeof TcsJob>,
  input: ParseTcsJobsInput,
  ...
  const id = jobId({ ats: "tcs", tenant_slug: input.tenant.slug, source_id: sourceId, url });
...
export function parseTcsJobs(input: ParseTcsJobsInput): Job[] {
  const parsed = TcsResponse.parse(input.response);
...
export interface ScrapeTcsOptions {
  readonly tenant: TenantInput;
...
export interface ScrapeTcsOutcome {
  readonly jobs: ReadonlyArray<Job>;
  readonly result: TenantResult;
}

export async function scrapeTcsTenant(opts: ScrapeTcsOptions): Promise<ScrapeTcsOutcome> {
  if (opts.tenant.slug !== TENANT_SLUG) {
    return {
      ...
      result: {
        ...
        status: "dead",
        error: `tcs is a single-tenant ATS, slug must be "${TENANT_SLUG}"`,
        jobs_count: 0,
      },
    };
  }
  ...
  let lastStatus = 0;
  const company = opts.tenant.display_name ?? "Tata Consultancy Services";
  try {
    for (let page = 1; page <= maxPages; page++) {
      ...
      const parsed = TcsResponse.parse(body);
      if (typeof parsed.total === "number") total = parsed.total;
      const pageJobs = parseTcsJobs({
        tenant: opts.tenant,
        company,
        response: body,
