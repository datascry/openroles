...
// TotalEnergies public job-search endpoint.
//
// Endpoint: https://careers.totalenergies.com/api/jobs
// Method: GET
// Query: page, size
//
...
const TotalEnergiesResponse = z
  .object({
    total: z.number().optional(),
    jobs: z.array(TotalEnergiesJob).optional(),
    results: z.array(TotalEnergiesJob).optional(),
  })
  .passthrough();

const PAGE_SIZE = 100;
const DEFAULT_MAX_PAGES = 10;
const TENANT_SLUG = "totalenergies";
const HOST = "careers.totalenergies.com";
...
export interface ParseTotalEnergiesJobsInput {
...
function buildCandidate(
  raw: z.infer<typeof TotalEnergiesJob>,
  input: ParseTotalEnergiesJobsInput,
...
export function parseTotalEnergiesJobs(input: ParseTotalEnergiesJobsInput): Job[] {
  const parsed = TotalEnergiesResponse.parse(input.response);
...
export interface ScrapeTotalEnergiesOptions {
  readonly tenant: TenantInput;
...
export interface ScrapeTotalEnergiesOutcome {
  readonly jobs: ReadonlyArray<Job>;
  readonly result: TenantResult;
}

export async function scrapeTotalEnergiesTenant(
  opts: ScrapeTotalEnergiesOptions,
): Promise<ScrapeTotalEnergiesOutcome> {
  if (opts.tenant.slug !== TENANT_SLUG) {
    return {
      ...
      result: {
        ...
        status: "dead",
        error: `totalenergies is a single-tenant ATS, slug must be "${TENANT_SLUG}"`,
        jobs_count: 0,
      },
    };
  }
  ...
  let lastStatus = 0;
  const company = opts.tenant.display_name ?? "TotalEnergies";
  try {
    for (let page = 1; page <= maxPages; page++) {
      ...
      const parsed = TotalEnergiesResponse.parse(body);
      if (typeof parsed.total === "number") total = parsed.total;
      const pageJobs = parseTotalEnergiesJobs({
        tenant: opts.tenant,
        company,
        response: body,
