...
// Trader Joe's public job-search endpoint.
//
// Endpoint: https://www.traderjoes.com/api/career/jobs
// Method: GET
// Query: page, size
//
...
//
// Single-tenant; slug = `traderjoes`. Small board (typically 50-200
...
const TjResponse = z
  .object({
    ...
    jobs: z.array(TjJob).optional(),
    results: z.array(TjJob).optional(),
  })
  .passthrough();

const PAGE_SIZE = 100;
const DEFAULT_MAX_PAGES = 10;
const TENANT_SLUG = "traderjoes";
const HOST = "www.traderjoes.com";
...
export interface ParseTjJobsInput {
...
function buildCandidate(
  raw: z.infer<typeof TjJob>,
  input: ParseTjJobsInput,
...
export function parseTjJobs(input: ParseTjJobsInput): Job[] {
  const parsed = TjResponse.parse(input.response);
...
export interface ScrapeTjOptions {
  readonly tenant: TenantInput;
...
export interface ScrapeTjOutcome {
  readonly jobs: ReadonlyArray<Job>;
  readonly result: TenantResult;
}

export async function scrapeTraderJoesTenant(opts: ScrapeTjOptions): Promise<ScrapeTjOutcome> {
  if (opts.tenant.slug !== TENANT_SLUG) {
    return {
      ...
      result: {
        ...
        status: "dead",
        error: `traderjoes is a single-tenant ATS, slug must be "${TENANT_SLUG}"`,
        jobs_count: 0,
      },
    };
  }
  ...
  let lastStatus = 0;
  const company = opts.tenant.display_name ?? "Trader Joe's";
  try {
    for (let page = 1; page <= maxPages; page++) {
      if ((page - 1) * limit >= total) break;
      const res = await opts.client.request(
        `https://${HOST}/api/career/jobs?page=${page}&size=${limit}`,
        { method: "GET" },
      );
      lastStatus = res.status;
      const body = await res.json();
      const parsed = TjResponse.parse(body);
      if (typeof parsed.total === "number") total = parsed.total;
      const pageJobs = parseTjJobs({
        tenant: opts.tenant,
        company,
        response: body,
