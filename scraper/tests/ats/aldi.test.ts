...
import { parseAldiJobs, scrapeAldiTenant } from "../../src/ats/aldi.ts";
  ...
  jobs: [
    {
      ...
      url: `https://${HOST}/careers/job/ALDI-1`,
    },
  ],
};
...
const largeFixture = {
  total: 3,
  jobs: [
    { jobId: "ALDI-2001", title: "Store Manager", location: "Brooklyn, NY", country_code: "US" },
    {
      jobId: "ALDI-2002",
...
describe("parseAldiJobs", () => {
  it("parses small fixture and honors API URL", () => {
    const jobs = parseAldiJobs({
      tenant: { slug: "aldi" },
      company: "ALDI",
    ...
    expect(jobs[0]?.title).toBe("Store Associate");
    expect(jobs[0]?.location_country).toBe("US");
    expect(jobs[0]?.url).toBe(`https://${HOST}/careers/job/ALDI-1`);
  });
  ...
  it("is deterministic (property)", () => {
    fc.assert(
      fc.property(fc.constantFrom("aldi"), (slug) => {
        const a = parseAldiJobs({
          tenant: { slug },
          company: "ALDI",
          response: largeFixture,
          observedAt: OBSERVED_AT,
        });
        const b = parseAldiJobs({
          tenant: { slug },
          company: "ALDI",
          response: largeFixture,
          observedAt: OBSERVED_AT,
...
describe("scrapeAldiTenant", () => {
  it("rejects non-canonical slug", async () => {
    ...
    const out = await scrapeAldiTenant({
      tenant: { slug: "not-aldi" },
  ...
  it("paginates until total reached", async () => {
    ...
    const out = await scrapeAldiTenant({
      tenant: { slug: "aldi", display_name: "ALDI" },
      client: clientWithRobotsAllowAll(),
      observedAt: OBSERVED_AT,
    });
    expect(calls).toBe(2);
