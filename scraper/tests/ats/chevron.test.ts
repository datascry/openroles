...
import { parseChevronJobs, scrapeChevronTenant } from "../../src/ats/chevron.ts";
...
const largeFixture = {
  total: 3,
  jobs: [
    { jobId: "CVX-2001", title: "Store Manager", location: "Brooklyn, NY", country_code: "US" },
    {
      jobId: "CVX-2002",
    ...
    { jobId: "CVX-2003", title: "Remote Buyer (West)", location: "Remote, US", country_code: "US" },
  ],
};
...
describe("parseChevronJobs", () => {
  it("parses small fixture and honors API URL", () => {
    const jobs = parseChevronJobs({
      tenant: { slug: "chevron" },
      company: "Chevron",
  ...
  it("is deterministic (property)", () => {
    fc.assert(
      fc.property(fc.constantFrom("chevron"), (slug) => {
        const a = parseChevronJobs({
          tenant: { slug },
          company: "Chevron",
          response: largeFixture,
          observedAt: OBSERVED_AT,
        });
        const b = parseChevronJobs({
          tenant: { slug },
          company: "Chevron",
          response: largeFixture,
          observedAt: OBSERVED_AT,
...
describe("scrapeChevronTenant", () => {
  it("rejects non-canonical slug", async () => {
    ...
    const out = await scrapeChevronTenant({
      tenant: { slug: "not-chevron" },
  ...
  it("paginates until total reached", async () => {
    ...
    const out = await scrapeChevronTenant({
      tenant: { slug: "chevron", display_name: "Chevron" },
      client: clientWithRobotsAllowAll(),
      observedAt: OBSERVED_AT,
    });
    expect(calls).toBe(2);
