...
import { parseExxonMobilJobs, scrapeExxonMobilTenant } from "../../src/ats/exxonmobil.ts";
...
const largeFixture = {
  total: 3,
  jobs: [
    { jobId: "XOM-2001", title: "Store Manager", location: "Brooklyn, NY", country_code: "US" },
    {
      jobId: "XOM-2002",
    ...
    { jobId: "XOM-2003", title: "Remote Buyer (West)", location: "Remote, US", country_code: "US" },
  ],
};
...
describe("parseExxonMobilJobs", () => {
  it("parses small fixture and honors API URL", () => {
    const jobs = parseExxonMobilJobs({
      tenant: { slug: "exxonmobil" },
      company: "ExxonMobil",
  ...
  it("is deterministic (property)", () => {
    fc.assert(
      fc.property(fc.constantFrom("exxonmobil"), (slug) => {
        const a = parseExxonMobilJobs({
          tenant: { slug },
          company: "ExxonMobil",
          response: largeFixture,
          observedAt: OBSERVED_AT,
        });
        const b = parseExxonMobilJobs({
          tenant: { slug },
          company: "ExxonMobil",
          response: largeFixture,
          observedAt: OBSERVED_AT,
...
describe("scrapeExxonMobilTenant", () => {
  it("rejects non-canonical slug", async () => {
    ...
    const out = await scrapeExxonMobilTenant({
      tenant: { slug: "not-exxonmobil" },
