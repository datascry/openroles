...
import { parseHmGroupJobs, scrapeHmGroupTenant } from "../../src/ats/hmgroup.ts";
...
const largeFixture = {
  total: 3,
  jobs: [
    { jobId: "HM-2001", title: "Store Manager", location: "Brooklyn, NY", country_code: "US" },
    {
      jobId: "HM-2002",
    ...
    { jobId: "HM-2003", title: "Remote Buyer (West)", location: "Remote, US", country_code: "US" },
  ],
};
...
describe("parseHmGroupJobs", () => {
  it("parses small fixture and honors API URL", () => {
    const jobs = parseHmGroupJobs({
      tenant: { slug: "hmgroup" },
      company: "HTrader Joe'sM Group",
  ...
  it("is deterministic (property)", () => {
    fc.assert(
      fc.property(fc.constantFrom("hmgroup"), (slug) => {
        const a = parseHmGroupJobs({
          tenant: { slug },
          company: "HTrader Joe'sM Group",
          response: largeFixture,
          observedAt: OBSERVED_AT,
        });
        const b = parseHmGroupJobs({
          tenant: { slug },
          company: "HTrader Joe'sM Group",
          response: largeFixture,
          observedAt: OBSERVED_AT,
...
describe("scrapeHmGroupTenant", () => {
  it("rejects non-canonical slug", async () => {
    ...
    const out = await scrapeHmGroupTenant({
      tenant: { slug: "not-hmgroup" },
  ...
  it("paginates until total reached", async () => {
    ...
    const out = await scrapeHmGroupTenant({
      tenant: { slug: "hmgroup", display_name: "HTrader Joe'sM Group" },
      client: clientWithRobotsAllowAll(),
      observedAt: OBSERVED_AT,
    });
    expect(calls).toBe(2);
