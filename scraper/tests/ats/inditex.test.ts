...
import { parseInditexJobs, scrapeInditexTenant } from "../../src/ats/inditex.ts";
...
const largeFixture = {
  total: 3,
  jobs: [
    { jobId: "INDI-2001", title: "Store Manager", location: "Brooklyn, NY", country_code: "US" },
    {
      jobId: "INDI-2002",
...
describe("parseInditexJobs", () => {
  it("parses small fixture and honors API URL", () => {
    const jobs = parseInditexJobs({
      tenant: { slug: "inditex" },
      company: "Inditex",
  ...
  it("is deterministic (property)", () => {
    fc.assert(
      fc.property(fc.constantFrom("inditex"), (slug) => {
        const a = parseInditexJobs({
          tenant: { slug },
          company: "Inditex",
          response: largeFixture,
          observedAt: OBSERVED_AT,
        });
        const b = parseInditexJobs({
          tenant: { slug },
          company: "Inditex",
          response: largeFixture,
          observedAt: OBSERVED_AT,
...
describe("scrapeInditexTenant", () => {
  it("rejects non-canonical slug", async () => {
    ...
    const out = await scrapeInditexTenant({
      tenant: { slug: "not-inditex" },
  ...
  it("paginates until total reached", async () => {
    ...
    const out = await scrapeInditexTenant({
      tenant: { slug: "inditex", display_name: "Inditex" },
      client: clientWithRobotsAllowAll(),
      observedAt: OBSERVED_AT,
    });
    expect(calls).toBe(2);
