  ...
  results: [
    {
      ...
      url: `https://${HOST}/job/INFY-2026-12345`,
    },
  ],
};
...
describe("parseInfosysJobs", () => {
  it("parses the small fixture and honors API-provided URL", () => {
    ...
    expect(jobs[0]?.url).toBe(`https://${HOST}/job/INFY-2026-12345`);
  });
  ...
  it("is deterministic (property)", () => {
    fc.assert(
      fc.property(fc.constantFrom("infosys"), (slug) => {
        const a = parseInfosysJobs({
          tenant: { slug },
          company: "Infosys",
          response: largeFixture,
          observedAt: OBSERVED_AT,
        });
        const b = parseInfosysJobs({
          tenant: { slug },
          company: "Infosys",
          response: largeFixture,
          observedAt: OBSERVED_AT,
...
describe("scrapeInfosysTenant", () => {
  it("rejects non-canonical slug without making a request", async () => {
    let calls = 0;
    server.use(
      http.get(`https://${HOST}/jobsearch`, () => {
        calls += 1;
        return HttpResponse.json({ total: 0, results: [] });
      }),
    );
    const out = await scrapeInfosysTenant({
      tenant: { slug: "not-infosys" },
  ...
  it("paginates until total reached", async () => {
    let calls = 0;
    server.use(
      http.get(`https://${HOST}/jobsearch`, ({ request }) => {
        calls += 1;
    ...
    const out = await scrapeInfosysTenant({
      tenant: { slug: "infosys", display_name: "Infosys" },
      client: clientWithRobotsAllowAll(),
      observedAt: OBSERVED_AT,
    });
    expect(calls).toBe(2);
  ...
  it("classifies 5xx as transient_failure", async () => {
    server.use(
      http.get(`https://${HOST}/jobsearch`, () => HttpResponse.text("oops", { status: 503 })),
    );
    const out = await scrapeInfosysTenant({
      tenant: { slug: "infosys" },
      client: clientWithRobotsAllowAll(),
