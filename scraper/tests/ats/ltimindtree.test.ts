...
import { parseLtiJobs, scrapeLtimindtreeTenant } from "../../src/ats/ltimindtree.ts";
...
describe("parseLtiJobs", () => {
  it("parses the small fixture and honors API-provided URL", () => {
    const jobs = parseLtiJobs({
      tenant: { slug: "ltimindtree" },
      company: "LTIMindtree",
  ...
  it("is deterministic (property)", () => {
    fc.assert(
      fc.property(fc.constantFrom("ltimindtree"), (slug) => {
        const a = parseLtiJobs({
          tenant: { slug },
          company: "LTIMindtree",
          response: largeFixture,
          observedAt: OBSERVED_AT,
        });
        const b = parseLtiJobs({
          tenant: { slug },
          company: "LTIMindtree",
          response: largeFixture,
          observedAt: OBSERVED_AT,
...
describe("scrapeLtimindtreeTenant", () => {
  it("rejects non-canonical slug without making a request", async () => {
    ...
    const out = await scrapeLtimindtreeTenant({
      tenant: { slug: "not-ltimindtree" },
  ...
  it("paginates until total reached", async () => {
    ...
    const out = await scrapeLtimindtreeTenant({
      tenant: { slug: "ltimindtree", display_name: "LTIMindtree" },
      client: clientWithRobotsAllowAll(),
      observedAt: OBSERVED_AT,
    });
    expect(calls).toBe(2);
