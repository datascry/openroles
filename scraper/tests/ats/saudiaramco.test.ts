...
import { parseSaudiAramcoJobs, scrapeSaudiAramcoTenant } from "../../src/ats/saudiaramco.ts";
...
const largeFixture = {
  total: 3,
  jobs: [
    { jobId: "ARA-2001", title: "Store Manager", location: "Brooklyn, NY", country_code: "US" },
    {
      jobId: "ARA-2002",
    ...
    { jobId: "ARA-2003", title: "Remote Buyer (West)", location: "Remote, US", country_code: "US" },
  ],
};
...
describe("parseSaudiAramcoJobs", () => {
  it("parses small fixture and honors API URL", () => {
    const jobs = parseSaudiAramcoJobs({
      tenant: { slug: "saudiaramco" },
  ...
  it("is deterministic (property)", () => {
    fc.assert(
      fc.property(fc.constantFrom("saudiaramco"), (slug) => {
        const a = parseSaudiAramcoJobs({
          tenant: { slug },
          company: "Saudi Aramco",
          response: largeFixture,
          observedAt: OBSERVED_AT,
        });
        const b = parseSaudiAramcoJobs({
          tenant: { slug },
          company: "Saudi Aramco",
          response: largeFixture,
          observedAt: OBSERVED_AT,
...
describe("scrapeSaudiAramcoTenant", () => {
  it("rejects non-canonical slug", async () => {
    ...
    const out = await scrapeSaudiAramcoTenant({
      tenant: { slug: "not-saudiaramco" },
  ...
  it("paginates until total reached", async () => {
    ...
    const out = await scrapeSaudiAramcoTenant({
      tenant: { slug: "saudiaramco", display_name: "Saudi Aramco" },
      client: clientWithRobotsAllowAll(),
      observedAt: OBSERVED_AT,
    });
    expect(calls).toBe(2);
