...
import { parse7eJobs, scrapeSevenElevenTenant } from "../../src/ats/seveneleven.ts";
...
const largeFixture = {
  total: 3,
  jobs: [
    { jobId: "7E-2001", title: "Store Manager", location: "Brooklyn, NY", country_code: "US" },
    {
      jobId: "7E-2002",
    ...
    { jobId: "7E-2003", title: "Remote Buyer (West)", location: "Remote, US", country_code: "US" },
  ],
};
...
describe("parse7eJobs", () => {
  it("parses small fixture and honors API URL", () => {
    const jobs = parse7eJobs({
      tenant: { slug: "seveneleven" },
      company: "7-Eleven",
  ...
  it("is deterministic (property)", () => {
    fc.assert(
      fc.property(fc.constantFrom("seveneleven"), (slug) => {
        const a = parse7eJobs({
          tenant: { slug },
          company: "7-Eleven",
          response: largeFixture,
...
describe("scrapeSevenElevenTenant", () => {
  it("rejects non-canonical slug", async () => {
    ...
    const out = await scrapeSevenElevenTenant({
      tenant: { slug: "not-seveneleven" },
  ...
  it("paginates until total reached", async () => {
    ...
    const out = await scrapeSevenElevenTenant({
      tenant: { slug: "seveneleven", display_name: "7-Eleven" },
      client: clientWithRobotsAllowAll(),
      observedAt: OBSERVED_AT,
    });
    expect(calls).toBe(2);
