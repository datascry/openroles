...
import { parseTotalEnergiesJobs, scrapeTotalEnergiesTenant } from "../../src/ats/totalenergies.ts";
...
const largeFixture = {
  total: 3,
  jobs: [
    { jobId: "TTE-2001", title: "Store Manager", location: "Brooklyn, NY", country_code: "US" },
    {
      jobId: "TTE-2002",
    ...
    { jobId: "TTE-2003", title: "Remote Buyer (West)", location: "Remote, US", country_code: "US" },
  ],
};
...
describe("parseTotalEnergiesJobs", () => {
  it("parses small fixture and honors API URL", () => {
    const jobs = parseTotalEnergiesJobs({
      tenant: { slug: "totalenergies" },
  ...
  it("is deterministic (property)", () => {
    fc.assert(
      fc.property(fc.constantFrom("totalenergies"), (slug) => {
        const a = parseTotalEnergiesJobs({
          tenant: { slug },
          company: "TotalEnergies",
          response: largeFixture,
          observedAt: OBSERVED_AT,
        });
        const b = parseTotalEnergiesJobs({
          tenant: { slug },
          company: "TotalEnergies",
          response: largeFixture,
          observedAt: OBSERVED_AT,
...
describe("scrapeTotalEnergiesTenant", () => {
  it("rejects non-canonical slug", async () => {
    ...
    const out = await scrapeTotalEnergiesTenant({
      tenant: { slug: "not-totalenergies" },
  ...
  it("paginates until total reached", async () => {
    ...
    const out = await scrapeTotalEnergiesTenant({
      tenant: { slug: "totalenergies", display_name: "TotalEnergies" },
      client: clientWithRobotsAllowAll(),
      observedAt: OBSERVED_AT,
    });
    expect(calls).toBe(2);
