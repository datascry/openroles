...
import { parseTjJobs, scrapeTraderJoesTenant } from "../../src/ats/traderjoes.ts";
...
const largeFixture = {
  total: 3,
  jobs: [
    { jobId: "TJ-2001", title: "Store Manager", location: "Brooklyn, NY", country_code: "US" },
    {
      jobId: "TJ-2002",
    ...
    { jobId: "TJ-2003", title: "Remote Buyer (West)", location: "Remote, US", country_code: "US" },
  ],
};
...
describe("parseTjJobs", () => {
  it("parses small fixture and honors API URL", () => {
    const jobs = parseTjJobs({
      tenant: { slug: "traderjoes" },
  ...
  it("is deterministic (property)", () => {
    fc.assert(
      fc.property(fc.constantFrom("traderjoes"), (slug) => {
        const a = parseTjJobs({
          tenant: { slug },
          company: "Trader Joe's",
          response: largeFixture,
...
describe("scrapeTraderJoesTenant", () => {
  it("rejects non-canonical slug", async () => {
    let calls = 0;
    server.use(
      http.get(`https://${HOST}/api/career/jobs`, () => {
        calls += 1;
        return HttpResponse.json({ total: 0, jobs: [] });
      }),
    );
    const out = await scrapeTraderJoesTenant({
      tenant: { slug: "not-traderjoes" },
  ...
  it("paginates until total reached", async () => {
    let calls = 0;
    server.use(
      http.get(`https://${HOST}/api/career/jobs`, ({ request }) => {
        calls += 1;
    ...
    const out = await scrapeTraderJoesTenant({
      tenant: { slug: "traderjoes", display_name: "Trader Joe's" },
      client: clientWithRobotsAllowAll(),
      observedAt: OBSERVED_AT,
    });
    expect(calls).toBe(2);
  ...
  it("classifies 5xx as transient_failure", async () => {
    server.use(
      http.get(`https://${HOST}/api/career/jobs`, () => HttpResponse.text("oops", { status: 503 })),
    );
    const out = await scrapeTraderJoesTenant({
      tenant: { slug: "traderjoes" },
      client: clientWithRobotsAllowAll(),
