...
all 54 adapters in `ATS_IDS`, not only these examples):
...
| jobscore | one GET `careers.jobscore.com/jobs/{slug}/feed.json` (shared host; tenant slug = first path segment after `/jobs/`) | JSON `{ company_name, company_code (== slug), jobs: [...] }`; every row carries full HTML `description`, `opened_date` (→ posted_at) + `last_updated_date` (→ updated_at), `location`, `department`, integer `job_status_id` (81 = open — filter to those; other values are filled/closed roles) and a canonical `detail_url` on the shared host (host-guarded; off-host falls back to a composed `/careers/{slug}/jobs/{url_slug\|id}` deep link). The feed returns every role in one call regardless of `?page=N` (no per-page cap), so no pagination and no detail fetch; unknown slug → clean 404 |
...
| talentbrew | GET `{metadata.host}/search-jobs?p=N` (per-brand vanity host, 1-based pages, 15/page), walked until a page renders zero job anchors, the collected count reaches `data-total-results`, or a 500-page cap | server-rendered HTML in several skins; the skin-agnostic invariant is the job anchor (opening `<a>` with both an `href="/job/…"` and a `data-job-id`). Title = the anchor's inner text (tags stripped, entities decoded); id = `data-job-id`; url = `{host}` + href. Optional per-row columns read by class from the block after each anchor: `job-location` / `search-results__job-info location` → location, `job-date-posted` / `search-results__job-info date` → posted_at (`Mon. DD, YYYY` or `MM/DD/YYYY`, clamped to ≤ observedAt), `job-brand` → department. Built from the listing alone, no detail fetch. Host = mandatory `metadata.host`, SSRF-guarded like phenom; a decommissioned brand redirects off-host → dead |
